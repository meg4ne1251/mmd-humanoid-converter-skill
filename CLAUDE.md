...
8. **パーツ分離** — 顔/髪/体/服/装飾などでメッシュを別オブジェクトに分離（最後に実施）。
9. **シェイプキーのリネーム** — `あ い う え お` → `a i u e o`。
10. **任意の仕上げ** — 末端ボーン削除、三角面→四角面、服で隠れる素体メッシュの作成。
11. 完了後 FBX 出力 → MotionBuilder でキャラクタライズ。
