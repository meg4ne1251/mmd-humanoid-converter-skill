...
- 最終形は **Claude スキルとして単一スキルリポジトリ** で配布する（リポジトリ＝1スキル）。**リポジトリ名は `mmd-humanoid-converter-skill`、スキル名（`SKILL.md` の `name:`）は `mmd-humanoid-converter`** とする（リポジトリ名にだけ `-skill` を付ける）。
