...
MMD（MikuMikuDance）の `.pmx` / `.pmd` モデルを、Blender 上できれいな **Humanoid リグ** に変換する **Claude スキル** です。Claude が **Blender MCP** を通じて Blender を操作し、変換作業を代行します。
...
- **Claude モデルは Opus を推奨。** Sonnet でも動作しますが、ボーン構造の判断やウェイト転送など複雑なステップで勘違いを起こす可能性がやや高くなります。
...
3. Claude がシーンを調査したあと、最初に **2 つの実行モード** を提示します。
